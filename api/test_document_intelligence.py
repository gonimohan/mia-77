...
def create_dummy_pdf(filename: str, text_content: str = "Sample PDF content."):
    ...
    try:
        ...
        print(
            "PyMuPDF (fitz) not installed, cannot create dummy PDF for testing text_processor. Skipping PDF creation."
        )
...
class TestTextProcessor(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dummy_csv_path = create_dummy_csv(
            "sample_csv", {"header1": ["growth strategy data1"], "header2": ["other data1"]}
        )
        cls.dummy_xlsx_path = create_dummy_xlsx(
            "sample_xlsx", {"Sheet1": {"colA": ["xlsx growth strategy"], "colB": ["value"]}}
        )
...
class TestAPIEndpoints(unittest.TestCase):
    ...
    def test_upload_file_success(self, mock_get_current_user, mock_db_ops, mock_text_processor_ops):
        ...
        with open(dummy_file_path, "rb") as f:
            response = client.post("/api/upload", files={"file": ("api_test_upload.txt", f, "text/plain")})
    ...
    def test_upload_file_invalid_type(self, mock_get_current_user, mock_db_ops, mock_text_processor_ops):
        ...
        with open(dummy_file_path, "rb") as f:
            response = client.post("/api/upload", files={"file": ("invalid_type.zip", f, "application/zip")})
