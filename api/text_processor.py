...
def extract_text_from_pdf(file_path: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        if not full_text.strip():
            ...
            full_text = (
                f"PDF document with {len(pdf.pages)} pages. Text extraction may require OCR for scanned documents."
            )
            word_count = count_words(full_text)
        ...
        for sheet_name in xls.sheet_names:
            try:
                ...
                for col in df.columns:
                    ...
                    col_text = col_text[col_text != ""].dropna()
                    if not col_text.empty:
                        sheet_text_content.append(
                            f"{col} data: {' '.join(col_text.tolist()[:100])}"
                        )  # Limit to 100 items
...
def extract_text_from_file(
    file_path_str: str, file_extension: str, mime_type: Optional[str] = None
...
def analyze_text_keywords(text_content: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"Keyword analysis complete. Quality: {text_quality}, Categories matched: {matched_categories}/{total_categories}"
    )
...
def extract_entities(text_content: str) -> Dict[str, List[str]]:
    ...
    """
    entities = {"emails": [], "urls": [], "phone_numbers": [], "dates": [], "companies": [], "currencies": []}
